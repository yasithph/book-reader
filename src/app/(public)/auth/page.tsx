        ...
        <div className="kindle-auth-content">
          {step === "phone" ? (
            <PhoneStep
              ...
              onSwitchToEmail={() => handleSwitchMode("email")}
              loading={loading}
              error={error}
            />
          ) : step === "email" ? (
            <EmailStep
              onSubmit={handleSendOTP}
              onSwitchToPhone={() => handleSwitchMode("phone")}
              loading={loading}
              error={error}
            />
          ) : step === "otp" ? (
            <OTPStep
              identifier={authMode === "email" ? email : phone}
              identifierType={authMode}
  ...
  return (
    <form onSubmit={handleSubmit} className="kindle-auth-form">
      ...
      <button
        ...
        className="kindle-auth-switch-mode"
      >
        Use email instead
      </button>
    </form>
  ...
  return (
    <form onSubmit={handleSubmit} className="kindle-auth-form">
      <div className="kindle-auth-field">
        <label className="kindle-auth-label">
          Email address
          <span className="kindle-auth-label-si sinhala">ඊමේල් ලිපිනය</span>
        </label>
        ...
        <p className="kindle-auth-hint sinhala">
          ඔබගේ ඊමේල් ලිපිනයට සත්‍යාපන කේතයක් යවනු ලැබේ
        </p>
      </div>
...
function OTPStep({
  ...
  const maskedIdentifier = identifierType === "email"
    ? identifier.replace(/^(.{1,2})(.*)(@.*)$/, (_, start, middle, domain) =>
        `${start}${"*".repeat(Math.min(middle.length, 6))}${domain}`
      )
    : `+94 ${identifier.replace(/(\d{2})(\d+)(\d{2})/, "$1****$3")}`;

  const backLabel = identifierType === "email" ? "Change email" : "Change number";
  ...
  return (
    <form onSubmit={handleSubmit} className="kindle-auth-form">
      ...
      <div className="kindle-auth-phone-display">
        ...
        <p className="kindle-auth-phone-number">{maskedIdentifier}</p>
      </div>
