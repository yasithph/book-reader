...
export function BottomNav({ isLoggedIn }: BottomNavProps) {
  ...
  // Don't show on reader pages or if not logged in
  if (!isLoggedIn || pathname.startsWith("/read/")) {
    return null;
  }
  ...
  return (
    <nav className="kindle-bottom-nav">
      ...
      >
        ...
        <span className="kindle-nav-label">Home</span>
      </Link>

      <Link
        href="/library"
        className={`kindle-nav-tab ${isActive("/library") ? "kindle-nav-tab-active" : ""}`}
      >
        <svg className="kindle-nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
        ...
        <span className="kindle-nav-label">Library</span>
      </Link>
      ...
      >
        ...
        <span className="kindle-nav-label">Leaderboard</span>
      </Link>
      ...
      >
        ...
        <span className="kindle-nav-label">More</span>
      </Link>
    </nav>
  );
}
